...
@app.route("/available_apps/twitter")
def twitter_login():
    if session:
        ...
        twitter_details = cursor.execute(
                    "SELECT * FROM socials WHERE user_id = (?) AND name = (?)", (session["user_id"], "Twitter")).fetchall()

        conn.close()

        if (twitter_details):
            access_token = twitter_details[0]["access_token"]
            access_token_secret = twitter_details[0]["access_token_secret"]
        else:
            # Get request token
